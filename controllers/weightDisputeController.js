...
const createDispute = async (req, res) => {
    ...
    try {
        ...
        const { ord_id, dispute_deduction, dispute_boxes } = req.body;
        if (!ord_id || !dispute_deduction || !((dispute_boxes instanceof Array) && dispute_boxes?.length )) {
            return res.status(400).json({ message: 'Invalid request' });
        }
        const transaction = await db.beginTransaction();
        const [dispute] = await transaction.query(`INSERT INTO WEIGHT_DISPUTES (ord_id, dispute_deduction) VALUES (?,?)`,[ord_id, dispute_deduction]);
